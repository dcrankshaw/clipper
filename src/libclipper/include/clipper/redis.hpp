...
bool send_cmd_no_reply(redox::Redox& redis, const std::vector<std::string>& cmd_vec) {
  bool ok = true;
  redox::Command<ReplyT>& cmd = redis.commandSync<ReplyT>(cmd_vec);
  if (!cmd.ok()) {
    log_error_formatted(LOGGING_TAG_REDIS, "Error with command \"{}\": {}", redis.vecToStr(cmd_vec),
                        cmd.lastError());
    ok = false;
  } else {
 ...
 */
std::string gen_model_replica_key(const VersionedModelId& key, const int model_replica_id);
 ...
 */
bool add_model(redox::Redox& redis, const VersionedModelId& model_id, const DataType& input_type,
               const std::vector<std::string>& labels, const std::string& container_name,
 ...
 */
std::unordered_map<std::string, std::string> get_model(redox::Redox& redis,
 ...
 */
std::unordered_map<std::string, std::string> get_container(redox::Redox& redis,
                                                           const VersionedModelId& model_id,
                                                           const int model_replica_id);
 ...
 * container was not found, an empty map will be returned.
 */
std::unordered_map<std::string, std::string> get_container_by_key(redox::Redox& redis,
                                                                  const std::string& key);
 ...
 * \return Returns true of the add was successful.
 */
bool add_application(redox::Redox& redis, const std::string& app_name, const DataType& input_type,
