...
class SelectionPolicy {
 ...
 public:
  ...
  virtual std::vector<PredictTask> select_predict_tasks(std::shared_ptr<SelectionState> state,
                                                        Query query, long query_id) const = 0;
  ...
  virtual std::pair<std::vector<PredictTask>, std::vector<FeedbackTask>> select_feedback_tasks(
      const std::shared_ptr<SelectionState>& state, FeedbackQuery query, long query_id) const = 0;
  ...
  virtual std::shared_ptr<SelectionState> deserialize(std::string serialized_state) const = 0;
  virtual std::string serialize(std::shared_ptr<SelectionState> state) const = 0;
};
...
class DefaultOutputSelectionState : public SelectionState {
 public:
  ...
  DefaultOutputSelectionState& operator=(const DefaultOutputSelectionState&) = default;
  DefaultOutputSelectionState(DefaultOutputSelectionState&&) = default;
  DefaultOutputSelectionState& operator=(DefaultOutputSelectionState&&) = default;
...
class DefaultOutputSelectionPolicy : public SelectionPolicy {
 public:
  ...
  DefaultOutputSelectionPolicy& operator=(const DefaultOutputSelectionPolicy&) = default;
  DefaultOutputSelectionPolicy(DefaultOutputSelectionPolicy&&) = default;
  DefaultOutputSelectionPolicy& operator=(DefaultOutputSelectionPolicy&&) = default;
  ~DefaultOutputSelectionPolicy() = default;
  ...
  std::vector<PredictTask> select_predict_tasks(std::shared_ptr<SelectionState> state, Query query,
                                                long query_id) const override;

  const std::pair<Output, bool> combine_predictions(const std::shared_ptr<SelectionState>& state,
                                                    Query query,
                                                    std::vector<Output> predictions) const override;

  std::pair<std::vector<PredictTask>, std::vector<FeedbackTask>> select_feedback_tasks(
      const std::shared_ptr<SelectionState>& state, FeedbackQuery query,
      long query_id) const override;

  std::shared_ptr<SelectionState> process_feedback(std::shared_ptr<SelectionState> state,
                                                   Feedback feedback,
                                                   std::vector<Output> predictions) const override;

  std::shared_ptr<SelectionState> deserialize(std::string serialized_state) const override;
