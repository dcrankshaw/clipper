...
TEST_F(DefaultOutputSelectionPolicyTest, TestSelectPredictTasksZeroCandidateModels) {
  Query zero_candidate_models_query{"label",
                                    ...
                                    {}};
  auto zero_models_tasks = policy_.select_predict_tasks(nullptr, zero_candidate_models_query, 0);
  EXPECT_EQ(zero_models_tasks.size(), (size_t)0);
}

TEST_F(DefaultOutputSelectionPolicyTest, TestSelectPredictTasksTwoCandidateModels) {
  std::vector<VersionedModelId> two_models{VersionedModelId("music_random_features", "1"),
  ...
  Query two_candidate_models_query{"label",
                                   ...
                                   DefaultOutputSelectionPolicy::get_name(),
                                   two_models};
  auto two_models_tasks = policy_.select_predict_tasks(nullptr, two_candidate_models_query, 0);
  EXPECT_EQ(two_models_tasks.size(), (size_t)1);
  EXPECT_EQ(two_models_tasks.front().model_, two_models.front());
}

TEST_F(DefaultOutputSelectionPolicyTest, TestSelectPredictTasksOneCandidateModel) {
  std::vector<VersionedModelId> one_model{VersionedModelId("music_random_features", "1")};
  Query one_candidate_model_query{"label",
                                  ...
                                  DefaultOutputSelectionPolicy::get_name(),
                                  one_model};
  auto one_model_tasks = policy_.select_predict_tasks(nullptr, one_candidate_model_query, 0);
  EXPECT_EQ(one_model_tasks.size(), (size_t)1);
  EXPECT_EQ(one_model_tasks.front().model_, one_model.front());
}

TEST_F(DefaultOutputSelectionPolicyTest, TestCombinePredictionsZeroPredictions) {
  ...
  Query one_candidate_model_query{"label",
                                  ...
                                  DefaultOutputSelectionPolicy::get_name(),
                                  {m1}};
  auto zero_preds_output = policy_.combine_predictions(state_, one_candidate_model_query, {}).first;
...
TEST_F(DefaultOutputSelectionPolicyTest, TestCombinePredictionsOnePrediction) {
  ...
  auto one_pred_output =
      policy_.combine_predictions(state_, one_candidate_model_query, {first_output}).first;
...
TEST_F(DefaultOutputSelectionPolicyTest, TestCombinePredictionsTwoPredictions) {
  ...
  auto two_preds_output =
      policy_.combine_predictions(state_, two_candidate_models_query, {first_output, second_output})
          .first;
