...
from ..container_manager import (
    ...
    CLIPPER_QUERY_FRONTEND_CONTAINER_LABEL, CLIPPER_INTERNAL_QUERY_PORT,
...
class GCPContainerManager(ContainerManager):
    ...
    def start_redis(self):
        startup_script = (
            ...
            "-p {redis_port}:6379 {image}").format(
                image="redis:alpine", redis_port=self.redis_port)

        redis_config = {
            "name":
            "redis-{}".format(self.cluster_name),
            "zone":
            "projects/clipper-model-comp/zones/us-west1-b",
            "minCpuPlatform":
            "Automatic",
            "machineType":
            "projects/clipper-model-comp/zones/us-west1-b/machineTypes/n1-standard-1",
            "metadata": {
                "items": [{
                    "key": "startup-script",
            ...
            "disks": [{
                ...
                "initializeParams": {
                    "sourceImage":
                    "projects/clipper-model-comp/global/images/clipper-core-docker",
                    "diskType":
                    "projects/clipper-model-comp/zones/us-west1-b/diskTypes/pd-standard",
                    "diskSizeGb":
                    "50"
                }
            }],
            ...
            "networkInterfaces": [{
                "network":
                "projects/clipper-model-comp/global/networks/default",
                "subnetwork":
                "projects/clipper-model-comp/regions/us-west1/subnetworks/default",
                "accessConfigs": [{
                    "name": "External NAT",
                    "type": "ONE_TO_ONE_NAT"
                }],
            ...
            "serviceAccounts": [{
                "email":
                "450655029092-compute@developer.gserviceaccount.com",
                "scopes": [
        ...
        instances = self.compute.instances().list(
        ...
        for inst in instances["items"]:
            if inst["name"] == "redis-{}".format(self.cluster_name):
                self.redis_internal_ip = inst["networkInterfaces"][0][
                    "networkIP"]
                self.redis_external_ip = inst["networkInterfaces"][0][
    ...
    def start_mgmt_frontend(self):
        startup_script = (
            ...
            mgmt_port=CLIPPER_INTERNAL_MANAGEMENT_PORT,
            image="gcr.io/clipper-model-comp/management_frontend:debug",
            redis_ip=self.redis_internal_ip,
            redis_port=self.redis_port)

        mgmt_config = {
            "name":
            "clipper-mgmt-{}".format(self.cluster_name),
            "zone":
            "projects/clipper-model-comp/zones/us-west1-b",
            "minCpuPlatform":
            "Automatic",
    ...
    def start_query_frontend(self):
        ...
        query_config = {
            "name":
            "clipper-query-{}".format(self.cluster_name),
            "zone":
            "projects/clipper-model-comp/zones/us-west1-b",
            "minCpuPlatform":
            "Automatic",
            "machineType":
            "projects/clipper-model-comp/zones/us-west1-b/machineTypes/custom-4-56320-ext",
            "metadata": {
                "items": [{
                    "key": "startup-script",
                    "value": startup_script
    ...
    def _start_instance(self, config):
        op = self.compute.instances().insert(
            project=self.project, zone=self.zone, body=config).execute()
        self._wait_for_op(op)

    def _delete_instance(self, rep_name):
        op = self.compute.instances().delete(
            project=self.project, zone=self.zone, instance=rep_name).execute()
        return op
    ...
    def connect(self):
        ...
        for inst in instances["items"]:
            if inst["name"] == "clipper-query-{}".format(self.cluster_name):
                self.query_frontend_internal_ip = inst["networkInterfaces"][0][
                    "networkIP"]
                self.query_frontend_external_ip = inst["networkInterfaces"][0][
                    "accessConfigs"][0]["natIP"]
            ...
            if inst["name"] == "clipper-mgmt-{}".format(self.cluster_name):
                self.mgmt_frontend_internal_ip = inst["networkInterfaces"][0][
                    "networkIP"]
                self.mgmt_frontend_external_ip = inst["networkInterfaces"][0][
                    "accessConfigs"][0]["natIP"]
    ...
    def _get_replicas(self, name, version):
        replicas = self.compute.instances().list(
    ...
    def _add_replica(self,
        ...
        config = {
            ...
            "Automatic",
            "machineType":
            "projects/clipper-model-comp/zones/us-west1-b/machineTypes/custom-{num_cpus}-{mem}".
            format(num_cpus=num_cpus, mem=mem),
            "metadata": {
                "items": [{
                    "key": "startup-script",
                    "value": startup_script
            ...
            "disks": [{
                ...
                "initializeParams": {
                    "sourceImage":
                    "projects/clipper-model-comp/global/images/clipper-k80-docker",
                    "diskType":
                    "projects/clipper-model-comp/zones/us-west1-b/diskTypes/pd-standard",
                    "diskSizeGb":
                    "50"
        ...
        if gpu_type is not None:
            if gpu_type in ["p100", "k80"]:
                config["guestAccelerators"] = [{
                    "acceleratorType":
                    "projects/clipper-model-comp/zones/us-west1-b/acceleratorTypes/nvidia-tesla-{gpu_type}".
                    format(gpu_type=gpu_type),
                    "acceleratorCount":
                    1
                }]
                config["disks"][0]["initializeParams"][
                    "sourceImage"] = "projects/clipper-model-comp/global/images/clipper-{gpu_type}-docker".format(
                        gpu_type=gpu_type)
            else:
                logger.error(
                    "{} is invalid gpu type. Starting replica without a GPU.".
                    format(gpu_type))
    ...
    def reset(self):
        ...
        qf_client.connect(
            self.query_frontend_internal_ip,
            username=username,
        ...
        sin, sout, serr = qf_client.exec_command(
            "sudo docker stop {}".format(qf_container_id))
        ...
        replicas = self.compute.instances().list(
            project=self.project,
            zone=self.zone,
            filter="labels.clipper-cluster eq {}".format(
                self.cluster_name)).execute()
        reps_to_start = []
        if "items" in replicas:
            for rep in replicas["items"]:
                if "clipper-model" in rep["labels"]:
                    ...
                    model_client.set_missing_host_key_policy(
                        paramiko.WarningPolicy())
                    model_client.connect(
                        rep["name"], username=username, key_filename=key_path)
                    sin, sout, serr = model_client.exec_command(
                        "sudo nvidia-docker ps -q")
                    containers = [l.strip() for l in sout]
                    if not len(containers) == 1:
                        raise ClipperException(
                            "Error resetting model replica {}".format(
                                rep["name"]))
                    model_container_id = containers[0]
                    sin, sout, serr = model_client.exec_command(
                        "sudo nvidia-docker stop {}".format(
                            model_container_id))
        ...
        while not restarted:
            ...
            sin, sout, serr = qf_client.exec_command(
                "sudo docker start {}".format(qf_container_id))
            logger.info(
                "Starting query frontend. stdout: {sout}, stderr: {serr}".
                format(sout=sout.readlines(), serr=serr.readlines()))
            time.sleep(10)
            sin, sout, serr = qf_client.exec_command("sudo docker ps -q")
            ...
            if len(running_containers) == 1:
                ...
                logger.info(
                    "Problem starting query frontend. Trying again.\nstdout: {sout}\nstderr: {serr}".
                    format(sout=sout.readlines(), serr=serr.readlines()))

        # Now restart the model replicas
        for model_client, model_container_id in reps_to_start:
            sin, sout, serr = model_client.exec_command(
                "sudo nvidia-docker start {}".format(model_container_id))
            logger.info(
                "Model should be running. stdout: {sout}, stderr: {serr}".
                format(sout=sout.readlines(), serr=serr.readlines()))
    ...
    def get_admin_addr(self):
        return "{host}:{port}".format(
            host=self.mgmt_frontend_external_ip,
            port=CLIPPER_INTERNAL_MANAGEMENT_PORT)

    def get_query_addr(self):
        # raise NotImplementedError
        return "{host}:{port}".format(
            host=self.query_frontend_internal_ip,
