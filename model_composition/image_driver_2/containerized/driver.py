...
VALID_MODEL_NAMES = [
    INCEPTION_MODEL_APP_NAME, OPENCV_FEATS_MODEL_APP_NAME,
...
def get_heavy_node_config(model_name,
    ...
    if model_name == OPENCV_FEATS_MODEL_APP_NAME:
        ...
        return driver_utils.HeavyNodeConfig(
            name=OPENCV_FEATS_MODEL_APP_NAME,
            input_type="floats",
    ...
    if model_name == OPENCV_SVM_MODEL_APP_NAME:
        ...
        return driver_utils.HeavyNodeConfig(
            name=OPENCV_SVM_MODEL_APP_NAME,
            input_type="ints",
...
class Predictor(object):
    def __init__(self):
        ...
        self.client.start()
        self.init_stats()
        self.stats = {"thrus": [], "p99_lats": [], "mean_lats": []}
...
class ModelBenchmarker(object):
    def __init__(self, config, queue):
        ...
        self.input_generator_fn = self._get_input_generator_fn(
            model_app_name=self.config.name)
    ...
    def run(self, duration_seconds=120):
        ...
        while True:
            ...
            if ((curr_time - start_time).total_seconds() > duration_seconds
                ) or (predictor.total_num_complete == 10000):
                break
            time.sleep(1)
    ...
    def _get_inception_input(self):
        input_img = np.array(
            np.random.rand(299, 299, 3) * 255, dtype=np.float32)
    ...
    def _get_opencv_svm_input(self):
        return np.array(
            np.random.rand(NUM_SIFT_FEATURES * SIFT_FEATURE_VEC_SIZE) * 20,
            dtype=np.int32)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=120,
        help=
        'The maximum duration of the benchmarking process in seconds, per iteration'
    )
    parser.add_argument(
        '-m',
        '--model_name',
        type=str,
        help=
        "The name of the model to benchmark. One of: 'inception', 'opencv-feats', 'opencv-svm'"
    )
    parser.add_argument(
        '-b',
        '--batch_sizes',
    ...
    for num_replicas in replica_num_confs:
        for cpus_per_replica in cpus_per_replica_confs:
            for batch_size in batch_size_confs:
                ...
                cl = ClipperConnection(DockerContainerManager(redis_port=6380))
                cl.connect()
                driver_utils.save_results([model_config], cl, all_stats,
