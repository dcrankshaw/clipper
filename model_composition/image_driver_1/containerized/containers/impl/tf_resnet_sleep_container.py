...
class TfResNetContainer(rpc.ModelContainerBase):
    ...
    def __init__(self, model_graph_path, model_ckpt_path):
        self.outputs = [np.array(np.random.rand(2048) * 255, dtype=np.float32).flatten() for _ in range(1000)]
        self.idx = 0
        print("loading sleep model")
    ...
    def predict_floats(self, inputs):
        ...
        if self.idx + num_outputs > len(self.outputs):
            self.idx = 0

        outputs = self.outputs[self.idx : self.idx+num_outputs]
        self.idx += num_outputs
        time.sleep(0.15)
