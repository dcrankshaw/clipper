...
class TfResNetContainer(rpc.ModelContainerBase):
    ...
    def __init__(self, model_graph_path, model_ckpt_path):
        self.inputs = [np.array(np.random.rand(224, 224, 3) * 255, dtype=np.float32).flatten() for _ in range(1000)]
        self.idx = 0
    ...
    def predict_floats(self, inputs):
        ...
        if self.idx + num_inputs > len(self.inputs):
            self.idx = 0

        inputs = self.inputs[self.idx : self.idx+num_inputs]
        self.idx += num_inputs
