...
class Predictor(object):
    ...
    def predict(self, resnet_input, ksvm_input):
        ...
        def update_perf_stats():
            ...
            trial_length = max(800, 10 * self.batch_size)
        ...
        self.client.send_request(TF_KERNEL_SVM_MODEL_APP_NAME, ksvm_input) \
            .then(svm_continuation)
...
class DriverBenchmarker(object):
    def __init__(self, configs, queue, client_num, latency_upper_bound):
        ...
        base_inputs = [(self._get_resnet_input(), self._get_ksvm_input()) for _ in range(1000)]
    ...
    def initialize_request_rate(self):
        ...
        while len(predictor.stats["thrus"]) < 6:
            resnet_input, ksvm_input = self.inputs[idx]
            predictor.predict(resnet_input, ksvm_input)
            time.sleep(self.delay)
            idx += 1
            idx = idx % len(self.inputs)

        max_thruput = np.mean(predictor.stats["thrus"][1:])
        self.delay = 1.0 / max_thruput - 0.004
...
if __name__ == "__main__":
    ...
    for ksvm_reps in range(1, 16):
        ...
        configs = [
            # setup_inception(batch_size=inception_batch_size,
            ...
            #               allocated_gpus=[]),
            setup_kernel_svm(batch_size=ksvm_batch_size,
        ...
        fname = "ksvm_{}".format(ksvm_reps)
        driver_utils.save_results(configs, cl, all_stats,
                                  "just_ksvm_batch_size_{}".format(ksvm_batch_size),
                                  prefix=fname)
