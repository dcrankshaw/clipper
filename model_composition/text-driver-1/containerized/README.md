...
prediction request will contain an input of the type expected by the model being benchmarked (i.e. a movie review represented
as a python string for the LSTM model)
...
If you're benchmarking either the LSTM or Autocompletion model on a GPU, you can make sure that the model is using the GPU via the 
